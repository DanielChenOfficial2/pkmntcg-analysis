...
def calc_one_shot(pkmn_data, all_pkmn_data):
  ...
  for index, enemy_pkmn in all_pkmn_data.iterrows():
    ...
    offensive_supporter_eligible_list = []
    for i in range(len(offensive_supporters_df)):
      offensive_supporter = offensive_supporters_df.iloc[i]
      if not isinstance(offensive_supporter['Condition'], float): # indicates nan, meaning no condition\)
        offensive_supporter_isEligible = can_use_card_test(enemy_pkmn, pkmn_data, ast.literal_eval(offensive_supporter['Condition']))
        if offensive_supporter_isEligible[0] == True:
          offensive_supporter_eligible_list.append(offensive_supporter)
      else:
        offensive_supporter_eligible_list.append(offensive_supporter)
    ...
    for j in range(len(enemy_pkmn_moves)):
      ...
      for offensive_supporter in offensive_supporter_eligible_list:
        dmg_dealt_add = 0
        if offensive_supporter['Supporter Name'] == 'Kieran' or offensive_supporter['Supporter Name'] == 'Grant':
            ...
            for m, defensive_poketool in enumerate(defensive_poketool_eligible_list):
              ...
              if damage != None:
                ...
                if damage >= hp:
                  ...
                  cur_one_shot_by['Enemy Supporter'] = offensive_supporter['Supporter Name']
  ...
  if len(pkmn_one_shots[0]) != 2:
    ...
    for index, cur_one_shot in enumerate(pkmn_one_shots[0]['Pkmn Gets One Shot By']):
      ...
      if index >= cur_enemy_poketool_skip_until_index:
        ...
        for index2, one_shot2 in enumerate(pkmn_one_shots[0]['Pkmn Gets One Shot By'][index:], index): # find duplicate entries
          if one_shot2['Enemy Pokemon Poketool'] == cur_enemy_poketool and index2 < cur_enemy_pkmn_move_skip_until_index:
